  ...
  .then((txt) => {
    ...
    if (data && data.data && data.data.user && data.data.user.pinnedItems && data.data.user.pinnedItems.nodes) {
      ...
      for (var i = 0; i < projects.length; i++) {
        var obj = projects[i];
        var langobjs = obj.languages ? obj.languages.nodes : [];  
        var newLangobjs = [];
      ...
      fs.writeFile("./src/shared/opensource/projects.json", JSON.stringify(newProjects), function (err) {
        if (err) {
          console.log("Error occurred while writing to file:", JSON.stringify(err));
        }
      });
    } else {
      console.log("Error: Missing expected properties in the response data.");
    }
  })
  .catch((error) => {
